...
%roll the dice for fireball, then hp. and if you kill all the trolls it counts as a success. the probability is the number of
%successes over the number of times
...
%evaluates the probability at each num_to_find and creates an array, the pmf
...
%evaluates the cdf between begin and last. Sp it find the prob at each [num_to_find] and then adds it to the previous
...
%finds the average of [num_trials] dice rolls, each with [num_sides] sides, rolled [num_rolls] times
...
%finds the probability depending on the method of finding the probability
function prob = find_prob(num_trials, num_to_find, method, num_rolls, num_sides)
    succ_count=0;
    
    %checks to see if the number you look for (num_to_find) was gotten with the fun method, then counts the number of successes
    ...
    %The same thing as above, but without the fun method (i.e. you dont look for the max of 3 rolls)
    ...
    %probability that ability score = num_to_find for all 6 ability scores
...
%generates ability scores, if it does not match the number, it exits, returning a fail. 
%If all of them (i.e. num_scores) = num_to_get then it returns a success 
...
%rolls dice 3 times and takes the max value 
...
%rolls the dice with sides=num_sides and sums the results  
